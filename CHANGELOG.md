...
rebuilt there — IAM user `motionstudio-remotion` (policy generated by
`remotion lambda policies user`, not hand-written), role `remotion-lambda-role`,
the render function pinned to `--memory=2048 --disk=2048 --timeout=120` so it
reproduces the old function name, a fresh site bundle, and a new assets bucket
`motionstudio-media` (the old name `motionstudio-assets` is held by another
account now and returns 403 even to admin credentials). CORS on the new bucket
was reconstructed from the [2026-07-27] cache-poisoning entry, including the
`Accept-Ranges` / `Content-Range` expose-headers that video seeking needs.
`remotion lambda policies validate` passes all 30 permissions with the app
user's credentials, and an end-to-end Lambda render succeeded.

What the move exposed is the more interesting half. Every asset uploaded under
the old account kept 403ing, and nothing in the config could fix it: the bucket
name was in the database.

### Added
- **Cloud upload status on every asset tile.** Three states — spinner while
  uploading, tick when it lands, the existing amber `CloudOff` on failure —
  using `SaveIndicator`'s icon/colour vocabulary so cloud status reads the same
  across the app. In-flight state lives in a new session-only
  `engines/asset/uploadStatus.ts`, deliberately *not* on the `Asset`: that
  object is persisted, so a tab closed mid-upload would write
  `uploading: true` to storage and strand a permanent spinner. The tick fades
  after ~2.5s per `SaveIndicator`'s rule that a tick on settled work is noise.
- **Undo on asset removal.** `notifyUndoable` in `lib/noticeStore.tsx`, a
  `react-toastify` toast carrying the undo. `removeAsset` always went through
  `updateProject` with history on, and the blob is kept in IndexedDB precisely
  so ⌘Z restores a working file — nothing said so, which for a destructive
  single-click action is the same as it not existing. A toast rather than
  `showNotice`, which is single-slot: three removals in a row would collapse
  into one message and the first two would become unrecoverable.
- `navigator.storage.persist()`, requested from `putBlob` at the moment the
  origin first holds something worth keeping. Until an upload is confirmed the
  IndexedDB blob is the only copy, and best-effort storage is what a browser
  evicts first under disk pressure.

### Changed
- **Assets persist an S3 key, not a resolved URL.** `/api/upload-url` built
  `https://<bucket>.s3.<region>.amazonaws.com/<key>` and `engines/asset/store.ts`
  wrote that whole string onto the asset as `storageUrl`, freezing the bucket
  into every project's data. The key (`<assetId>.<ext>`, and nothing else) is
  the stable half, so it is what gets stored; the URL is rebuilt on read by the
  new `motionStudio/src/lib/assetUrl.ts`. A bucket change is an env var again.
- `Asset.storageKey` added; `Asset.storageUrl` kept as deprecated and
  migration-only, read by the healer and never written.
- All four readers go through `cloudUrl()`: `ExportDialog` (Lambda input props,
  plus the pending-asset count, which now tests `storageKey`),
...
- **Uploads retry instead of giving up after one attempt.** `lib/storage.ts`
  tried once; on failure `engines/asset/store.ts` recorded an `uploadError` and
  nothing ever retried, so one network blip at import time made a file
  permanently local-only — invisible, because the editor plays off the local
  blob, until the project was opened elsewhere or a render came back missing
  media. Now 3 attempts with ~1s/3s backoff, and only for what can change: our
  own API's 4xx are answers ("File exceeds 500 MB limit" does not become true on
  the third try), an S3 403 on the presigned PUT is a standing config condition,
  and 5xx/429/transport errors retry. While offline it waits for the `online`
  event (capped at 60s) rather than sleeping out a backoff whose answer cannot
  have changed.
- **`healCloudCopies` now repairs every asset without a `storageKey`**, not just
  legacy migration cases. Its old filter required a `storageUrl`, so an upload
  interrupted by a closed tab — no key, no legacy URL, no error — was skipped
  forever, as was anything that had failed and recorded an error. This is what
  makes the invariant hold: after a project opens, every asset with local bytes
  either has a `storageKey` or has an error saying why not. No silent third
  state, which is the only reason the tile's "no badge means it's in the cloud"
  is honest. The cheap rewrite case still runs first.
- **A successful PUT is verified with a `HEAD` before `storageKey` is written.**
  A 200 says S3 accepted the bytes, not that the app can read them back — public
  read comes from a bucket policy, and the read URL is built from a different env
  var than the one that signed the write. An asset claiming a cloud copy it does
  not have silently drops media from a render; one that admits it has none shows
  a badge and gets retried.
- **A failed upload now names the bucket it tried.** `Upload failed (403)` was
  true, useless, and identical whether the bucket was misconfigured, missing, or
...
- **`VITE_S3_ASSETS_BUCKET` was in the repo-root `.env`, which Vite never
  ...
  for every asset — media still played from IndexedDB, so the editor looked
  healthy while cloud renders shipped without media. Moved to
  `motionStudio/.env` (not `.env.local`, which the Vercel CLI owns and
  overwrites); README corrected. `lib/assetUrl.ts` now `console.error`s when the
  variable is missing instead of degrading in silence.
- **Assets orphaned by the account move are re-uploaded automatically.** New
  `engines/asset/healCloudCopies.ts`, called from the end of `rehydrateAssets`
  (not from its two callers in `App.tsx` and `EditorPage.tsx` — it is meaningless
  before the blob store has been read, and both entry points need it). Three
  cases, in order of cost: a legacy URL still pointing at the current bucket is
  rewritten to a `storageKey` with no network at all, which is what stops anyone
  who never hit the migration re-uploading their whole library; a URL pointing
  elsewhere with bytes in IndexedDB is re-uploaded from a `File` rebuilt off the
  blob; neither, and an `uploadError` is recorded. `putBlob` had kept every
  imported file locally all along, so on the device that did the original upload
  nothing was truly lost — only the remote copy, which is precisely what a cloud
  render needs.
- Uploads run sequentially rather than `Promise.all`, unlike the import path:
  this is background repair competing with someone trying to edit, and
  re-uploading a library of video in parallel would take the bandwidth playback
  needs. Patches go through `updateProject` with `{ history: false }` — repairing
  a broken link is not a user edit — and the project is re-read between uploads,
  since one takes seconds during which assets may be renamed or deleted.
- The unrepairable case is now visible. It previously stayed silent until a
  cloud render came back missing the media, which is the exact failure
