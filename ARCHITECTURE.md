...
| **Playwright** (devDependency) | Build-time only, two jobs: `scripts/prerender.mjs` snapshots real rendered HTML for `/` and `/contact` post-`vite build` (this is a client-side-only SPA with no server render — without this, most non-Google crawlers and every link-preview bot see an empty `<div id="root">`), and `scripts/generate-og-image.mjs` screenshots a brand-styled static card to produce `og-image.png`. Chosen over Puppeteer for exactly this render-and-snapshot shape, and because one browser instance covers both jobs. Never ships to the client. |
